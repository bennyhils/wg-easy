  ...
  constructor() {
    ...
    this.app = express()
      ...
      .post('/api/wireguard/client', Util.promisify(async req => {
        const { name } = req.body;
        const { tgLogin } = req.body;
        const { tgId } = req.body;
        const { tgFirst } = req.body;
        const { tgLast } = req.body;
        return WireGuard.createClient({ name, tgLogin, tgId, tgFirst, tgLast });
      }))
      ...
      .put('/api/wireguard/client/:clientId/paid', Util.promisify(async req => {
        const { clientId } = req.params;
        const { paidBefore } = req.body;
        return WireGuard.updateClientPaidBefore({ clientId, paidBefore });
      }))
